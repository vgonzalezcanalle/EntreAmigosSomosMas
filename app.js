...
const I18N = {
  ...
  en: {
    ...
    inicio_lema:'United for those who need it most', inicio_quienes_title:'Who are we?', inicio_p1:'We are a group of young people who, faced with the emergency caused by the earthquake in Venezuela, decided to take action and join forces to help the affected communities.', inicio_p2:'This initiative aims to bring together friends, family, and anyone else who wants to help but—being abroad or far from the affected areas—doesn't know how to go about it.', inicio_p3:'We work in direct contact with those on the front lines responding to the emergency and supporting affected communities; this allows us to identify actual needs and deliver aid quickly and directly.', inicio_p4:'Through this platform, we will share details of every purchase, delivery, and action taken, ensuring complete transparency regarding the use of every donation received.',, donor_title:'How do you want to help?',
  ...
  fr: {
    ...
    inicio_lema:'Unis pour ceux qui en ont le plus besoin', inicio_quienes_title:'Qui sommes-nous ?', inicio_p1:'Nous sommes un groupe de jeunes qui, face à l'urgence causée par le tremblement de terre au Venezuela, a décidé d'agir et d'unir ses forces pour aider les communautés touchées.', inicio_p2:'Cette initiative vise à rassembler des amis, des familles et toute personne souhaitant aider mais qui, étant à l'étranger ou loin des zones touchées, ne sait pas comment s'y prendre.', inicio_p3:'Nous travaillons en contact direct avec ceux qui sont en première ligne, ce qui nous permet d'identifier les besoins réels et d'acheminer l'aide rapidement et directement.', inicio_p4:'À travers cette plateforme, nous partagerons les détails de chaque achat, livraison et action menée, garantissant une transparence totale dans l'utilisation de chaque don reçu.',, 
  ...
  it: {
    ...
    inicio_lema:'Uniti per chi ne ha più bisogno', inicio_quienes_title:'Chi siamo?', inicio_p1:'Siamo un gruppo di giovani che, di fronte all'emergenza causata dal terremoto in Venezuela, ha deciso di agire e unire le forze per aiutare le comunità colpite.', inicio_p2:'Questa iniziativa mira a riunire amici, familiari e chiunque voglia aiutare ma che, trovandosi all'estero o lontano dalle zone colpite, non sa come farlo.', inicio_p3:'Lavoriamo in contatto diretto con chi è in prima linea, il che ci permette di identificare i bisogni reali e consegnare aiuti rapidamente e direttamente.', inicio_p4:'Attraverso questa piattaforma condivideremo i dettagli di ogni acquisto, consegna e azione intrapresa, garantendo piena trasparenza sull'utilizzo di ogni donazione ricevuta.',, 
...
function setLang(lang) {
  LANG = lang;
  // Soporte para árabe de Derecha a Izquierda (RTL)
  if (lang === 'ar') {
...
async function submitNecesidad(e) {
  ...
  if (!cat || !desc || !cant || !ciudad || !pin || pin.length !== 4) { showToast(t('toast_error')); return; }
  const pin_hash = await hashPin(pin);
  DB.add('insumos', { id: uuid(), categoria: cat, descripcion: desc, cantidad: cant, unidad: unid, ciudad, estado_vzla: ciudad, hospital_centro: hosp, lat, lng, tipo_sangre: tSang, estado_ayuda: 'necesitado', pin_hash, foto_factura_url: null, id_transporte: null, idioma_publicacion: LANG, creado_en: now(), actualizado_en: now() });
